...
$(document).ready(function () {
  ...
  //when game ends this will clear timer and stop showing moles
  function endGame() {
    clearInterval(timer);
  ...
  // used help creating the timer. Gave credit in README file
  function updateTimer() {
    //responsible for subtracting time left by 1
    timeLeft = timeLeft - 1;
    if (timeLeft >= 0) {
      //time left greater than or equal to 0 then show time
  ...
  function handleWhack() {
    ...
    if (moleHole.hasClass("active")) {
      //if you whack a mole then it will give you a class of active
  ...
  function updateScoreDisplay() {
    const scoreElement = $(".score"); // $(".score") will show the current score
